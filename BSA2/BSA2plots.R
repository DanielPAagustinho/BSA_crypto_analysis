...
load(file = "/Users/danielagustinho/GoogleDrive/WashU/DNA/EXP#054/Rdata.RData")
sourcce("/Volumes/SEAGATE_EXP/WashU/DNA/BSAanalysis/ScriptTemplates/BSA6tools.R")
...
samplesNonSmooth=lapply(c(1,2), FUN = function(x){
  data.frame(CHROM=allPools[[names(allPools)[x]]]$CHROM, 
             binFloor=allPools[[names(allPools)[x]]]$binFloor,
             binCeiling=allPools[[names(allPools)[x]]]$binCeiling,
             binMiddle=allPools[[names(allPools)[x]]]$binMiddle,
             Bin_size=allPools[[names(allPools)[x]]]$Bin_size,
             P1=Pools[[paste0("P1.",names(allPools)[x])]]$deltaSNP,
             P1sig=Pools[[paste0("P1.",names(allPools)[x])]]$Significance,
             P2=Pools[[paste0("P2.",names(allPools)[x])]]$deltaSNP,
             P2sig=Pools[[paste0("P2.",names(allPools)[x])]]$Significance,
             P3=Pools[[paste0("P3.",names(allPools)[x])]]$deltaSNP,
             P3sig=Pools[[paste0("P3.",names(allPools)[x])]]$Significance,
             P4=Pools[[paste0("P4.",names(allPools)[x])]]$deltaSNP,
             P4sig=Pools[[paste0("P4.",names(allPools)[x])]]$Significance,
             P5=Pools[[paste0("P5.",names(allPools)[x])]]$deltaSNP,
             P5sig=Pools[[paste0("P5.",names(allPools)[x])]]$Significance,
...
names(samplesNonSmooth)=names(allPools)
names(samplesNonSmooth)=names(allPools)


#This is a table with the Max and Min of ∆SNP for each position.
MaxAndMins=cbind(allPoolsInOne_filteredBSA4[["Y"]][,1:4], data.frame(Min_SNPchange=-(allPoolsInOne_filteredBSA4[["Y"]]$SNPindex.LOW),
                                                                     Max_SNPchange=1-(allPoolsInOne_filteredBSA4[["Y"]]$SNPindex.LOW))  ) #The SNPindex.LOW should be the same for YPD and Lungs.... 


MaxAndMins=MaxAndMins %>% filter(!(CHROM=="chr2" & POS==283162)) #This mutation was erased in the addition of the drug marker in the C8 parent (It was in the arm).
...
#    geom_line(data = newMinsAndMax, aes(x=POS/1000, y=Max_SNPchange))+
  ...
  ggsave(p1,filename = paste0("/Users/danielagustinho/GoogleDrive/WashU/DNA/EXP#054/Plots/allChromosomes_nonSmoothed_chr", i, ".5kb.pdf"), dpi=500,height = 9,
         units = "in", width = 16)
...
#  geom_line(data = newMinsAndMax, aes(x=POS/1000, y=Max_SNPchange))+
  ...
  scale_size_manual(name="Condition and significance",
                    labels=c("Lung, not significant","YPD, not significant","Lung, significant",  "YPD, significant"),
                    values = c(0.05,0.05,0.2, 0.2), drop=F)+
  scale_colour_manual(name="Condition and significance",
                      labels=c("Lung, not significant","YPD, not significant","Lung, significant",  "YPD, significant"),
                      values = c('FALSE_Lungs'="red",'FALSE_YPD'="black",
                                 'TRUE_Lungs'="red",'TRUE_YPD'="black"), drop=F)+
  scale_alpha_manual(name="Condition and significance",
                     labels=c("Lung, not significant","YPD, not significant","Lung, significant",  "YPD, significant"),
                     values = c(0.7,0.7,1, 1), drop=F)+
#  geom_vline(xintercept  = c(283.162,	468.855),color="black", size=0.3, linetype="dashed")+
#  geom_vline(xintercept = 355.352, size=0.3, color="green")+
#  geom_vline(xintercept = 384.363, size=0.3, color="blue")+
#  geom_vline(xintercept = c(297.636, 355.352), size=0.3, color="brown", linetype="dashed")+
  labs(title=paste0("Chromosome ", i), x=paste0("Position on chromosome ",i," (kb)"),y="Change in allele frequency (BSA)", color="Pool")+
  geom_hline(yintercept = c(0), color="black", size=0.6)+
  scale_x_continuous(breaks = seq(from=0, to=900, by=300), limits=c(0, 950),expand = c(0, 0))+
  scale_y_continuous(breaks = c(-0.5, -0.25, 0,0.25,0.5,0.75, 1.0), limits = c(-0.6,1.1), expand = c(0,0))+
  theme(#legend.title = element_text(size=18), 
...
ggsave(p1,filename = "/Volumes/SEAGATE_EXP/WashU/DNA/BSAanalysis/BSA4/IR-1_refinement_noLines_5kbwindows.pdf", dpi = 500 )
ggsave(p1,filename = "/Volumes/SEAGATE_EXP/WashU/DNA/BSAanalysis/BSA4/IR-1_refinement_noLines_5kbwindows.tiff", dpi = 500 )

ggsave(p1,filename = paste0("/Users/danielagustinho/GoogleDrive/WashU/DNA/EXP#054/Plots/IR-1_5kb_NONsmoothed.pdf"), dpi=500,height = 9,
       units = "in", width = 16)

#############################################################################
test=rbind(samplesNonSmooth[["L"]], samplesNonSmooth[["Y"]]) %>%
  filter(CHROM!="chrM")
test$CHROM=gsub(pattern = "chr",replacement = "Chromosome ",x = test$CHROM)
test$CHROM=factor(test$CHROM, levels=unique(test$CHROM))
test$combine=paste(test$Significance, test$Condition, sep="_")
levels(test$combine)=c("FALSE_Lungs", "FALSE_YPD", "TRUE_Lungs","TRUE_YPD")
eachPool=c("P1sig","P2sig","P3sig","P4sig","P5sig")
PoolsCombine=lapply(eachPool, FUN=function(x){z=paste(test[,grep(x,colnames(test))], test$Condition, sep="_");return(z)   })

test=filter(test, CHROM %in% paste0("Chromosome ", 11:14))

p3=ggplot(data=test,aes(x=samplesNonSmooth$binMiddle/1000, y=test$All_pools))+
  #    geom_line(data = newMinsAndMax, aes(x=POS/1000, y=Min_SNPchange))+
  #    geom_line(data = newMinsAndMax, aes(x=POS/1000, y=Max_SNPchange))+
  geom_pointrange(data=test,
                  aes(ymin=pool_min, 
                      ymax=pool_max ,
                      x=binMiddle/1000, 
                      y=All_pools,
  ...
  scale_size_manual(name="Condition and significance",
                    labels=c("Lung, not significant","YPD, not significant","Lung, significant",  "YPD, significant"),
                    values = c(0.01,0.01,0.04, 0.04), drop=F)+
  scale_colour_manual(name="Condition and significance",
                      labels=c("Lung, not significant","YPD, not significant","Lung, significant",  "YPD, significant"),
                      values = c('FALSE_Lungs'="red",'FALSE_YPD'="black",
                                 'TRUE_Lungs'="red",'TRUE_YPD'="black"), drop=F)+
  ...
  #geom_vline(xintercept  = IR_list[[paste0("chr",i)]],color="black", size=0.3, linetype="dashed")+
  #geom_vline(xintercept = 0, size=0.3)+
  labs(title=paste0("Chromosome ", i), x=paste0("Position on chromosomes (kb)"),y="Change in allele frequency (BSA)", color="Pool")+
  geom_hline(yintercept = c(0), color="black", size=0.6)+
  scale_x_continuous(breaks = seq(from=0, to=2000, by=500), limits=c(0, 2300),expand = c(0, 0))+
  scale_y_continuous(breaks = c(-0.5, 0,0.5), limits = c(-0.6,0.6), expand = c(0,0))+
  ...
  theme(#legend.title = element_text(size=18), 
    ...
    axis.title.y =element_text(margin = margin(r=25, t=0, l=5, b=0), vjust=-5),
    axis.title.x =element_text(margin = margin(r=0, t=25, l=0, b=5), vjust=6),
...
ggsave(p3,filename = "/Volumes/SEAGATE_EXP/WashU/DNA/BSAanalysis/BSA4/BSA4-Chroms11-14.pdf", dpi = 500 )
ggsave(p3,filename = "/Volumes/SEAGATE_EXP/WashU/DNA/BSAanalysis/BSA4/BSA4-Chroms11-14.tiff", dpi = 500 )
